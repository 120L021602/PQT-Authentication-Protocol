 ...
 *	str_en	
 *	key	16 uint8_t
 *	str 	128 unsigned char
 */
void sm4_ecb_encryption(uint8_t *str, size_t len, uint8_t *key, uint8_t *str_en)
{
	struct sm4_context ctx;
    	sm4_setkey_enc(&ctx, key);
    	sm4_crypt_ecb(&ctx, len, str, str_en);
}
 ...
 *	str 	128 unsigned char
 */
void sm4_ecb_decryption(uint8_t *str_en, size_t enlen, uint8_t *key, uint8_t *str)
{
	struct sm4_context ctx;
    sm4_setkey_dec(&ctx, key);
	sm4_crypt_ecb(&ctx, enlen, str_en, str);
}
