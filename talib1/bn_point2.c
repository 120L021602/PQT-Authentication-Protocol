...
/*
   打印显示BNPoint；
 */
void P2_toString_d(BNPoint2 p, uint32_t system)
{
    if(P2_isZero(p))
        printf("0\n");
    else
...
/*
   ...
   str_de   char[1024]  初始化值0
 */
void P2_toString(BNPoint2 p, char *str, uint32_t system)
{
    char *s1,*s2;
    ...
    {
        s1 = Put(p.x.im, system); s2 = Format_64_Char(s1); strcpy(str, s2);
        s1 = Put(p.x.re, system); s2 = Format_64_Char(s1); strcpy(str+64, s2);
        s1 = Put(p.y.im, system); s2 = Format_64_Char(s1); strcpy(str+128, s2);
        s1 = Put(p.y.re, system); s2 = Format_64_Char(s1); strcpy(str+192, s2);
    }
}
